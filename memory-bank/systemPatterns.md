...
- **外部素材入库**:第三方图/svg 先核验许可(如引入则建 `src/assets/CREDITS.md` 署名,素材移除时文件一并清理);网络下载的大 SVG 可能被截断,先验 XML 完整性(或改用 npm pack)
- **地理地图慎用**:世界地图涉及边界表达合规风险(未经官方审图),本项目已整体移除;如需恢复须选经审图的地图数据源
